...
void get_rectifier_maps(double a, Mat& map_left_x, Mat& map_left_y, Mat& map_right_x, Mat& map_right_y, Rect& roi_left, Rect& roi_right) {
    // Create camera parameters - Numbers come from matlab calibration
    ...
    stereoRectify(
        ...
        R1, R2, P1, P2, Q,
        CALIB_ZERO_DISPARITY, a, Size(640, 480), 
        &roi_left, &roi_right
    );
...
int main() {
    ...
    const double focal_length_pixel = 855.0;
    ...
    try {
        ...
        auto remap2_duration = std::chrono::microseconds(0);

        CGrabResultPtr grabResultRight, grabResultLeft;

        cuda::GpuMat leftimg(480, 640, CV_8UC1), rightimg(480, 640, CV_8UC1);

        cuda::GpuMat map_x_left, map_y_left;
        map_x_left.upload(map_left_x);
        map_y_left.upload(map_left_y);

        cuda::GpuMat map_x_right, map_y_right;
        map_x_right.upload(map_right_x);
        map_y_right.upload(map_right_y);

        cuda::GpuMat color_leftimg(480, 640, CV_8UC3), color_rightimg(480, 640, CV_8UC3);
        cuda::GpuMat rect_leftimg(480, 640, CV_8UC3), rect_rightimg(480, 640, CV_8UC3);
        ...
        Mat yellowMask;
        Mat kernel_c, kernel_o, closing, opening;
        ...
        while (camera_left.IsGrabbing() && camera_right.IsGrabbing() && key != 27) {
            ...
            auto start_remap2 = std::chrono::high_resolution_clock::now();

            remap_color(color_left_image, rect_l_test, map_left);
            remap_color(color_right_image, rect_r_test, map_right);

            auto end_remap2 = std::chrono::high_resolution_clock::now();
            ...
            auto duration_remap2 = std::chrono::duration_cast<std::chrono::microseconds>(end_remap2 - start_remap2);
            remap2_duration += duration_remap2;
            ...
            for (const auto& contour : contours) {
                ...
                if (area > minAreaThreshold) { //&& area < maxAreaThreshold
                    ...
                    circle(rectified_left, Point(circ_centre.x+min_loc.x/4+10,circ_centre.y), 5, Scalar(255,0,0), FILLED);

                    disparity = 10.0 + (double)min_loc.x/4.0;
                    depth_mm = focal_length_pixel * baseline_mm / disparity;

                    double xmm, ymm, zmm=depth_mm;
                    xmm = zmm * (double)(circ_centre.x - 320) / 854.0;
                    ymm = zmm * (double)(240 - circ_centre.y) / 856.0;
            ...
            imshow("Left test", rect_l_test);
            imshow("Right test", rect_r_test);
        ...
        auto average_remap2 = remap2_duration / loops;
        std::cout << "Remap 2.0 timer: " << average_remap2.count() << " us" << std::endl;
