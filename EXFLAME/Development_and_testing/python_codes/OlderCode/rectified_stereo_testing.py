...
def main():
    ...
    # Load a new stereo image pair
    # left_image = cv.imread('Images/Images/Images/left/left_image_20.png')
    # right_image = cv.imread('Images/Images/Images/right/right_image_20.png')
    ...
    # cv.imshow('Left Image', rectified_left)
    ...
    # Load a new stereo image pair
    # left_image = cv.imread('Images/BayerBG/left_image_1.png', cv.IMREAD_GRAYSCALE)
    # right_image = cv.imread('Images/BayerBG/right_image_1.png', cv.IMREAD_GRAYSCALE)
    # comp_img = cv.imread('Images/Images/Images/left/left_image_20.png')
    
    # left_image = cv.cvtColor(left_image, cv.COLOR_BAYER_BG2RGB)
    # right_image = cv.cvtColor(right_image, cv.COLOR_BAYER_BGGR2BGR)
    
    # # Rectify the images
    # rectified_left = cv.remap(left_image, map_left_x, map_left_y, cv.INTER_LINEAR)
    # rectified_right = cv.remap(right_image, map_right_x, map_right_y, cv.INTER_LINEAR)  
    
    # cv.imshow("Base",left_image)
    # cv.imshow("Test",right_image)
    # cv.imshow("Comparison", comp_img)
    # cv.imshow("Rectified", rectified_left)
